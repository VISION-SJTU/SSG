...
class EvalDataset(Dataset):
    ...
    def load_dataset(self):
        if self.data_name == "coco2014" or "coco2017":
            if self.load_txt_from_file:
                ...
                num_images = len(self.imagelist)
                num_captions = len(captions)
                assert num_images % num_captions == 0, \
                    "Images must be evenly divisible by captions."

                num_images_per_caption = num_images // num_captions
                expanded_captions = [
                    cap for cap in captions
                    for _ in range(num_images_per_caption)
                ]
                assert len(expanded_captions) == num_images

                self.data = list(zip(self.imagelist, expanded_captions))
            else:
